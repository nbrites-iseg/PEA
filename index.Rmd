...
plot(time, W, type = "l", col = "blue", lwd = 2,
     ...
     main = "Exemplo de uma trajetória de um processo estocástico")
grid()
```
...
```{tikz, fig1, echo=FALSE, fig.align = 'center', out.width = '25%', fig.cap='Estado acessível'}
\begin{tikzpicture}[->, >=stealth, thick]

% Define manual nodes
\node[circle, draw, minimum size=.5cm] (A) at (0,0) {$i$};
\node[circle, draw, minimum size=.5cm] (B) at (3,0) {$j$};

% Draw arrow
\draw (A) -- node[midway, above] {~} (B);
...
```{tikz, fig2, echo=FALSE, fig.align = 'center', out.width = '25%', fig.cap='Estados em comunicação'}
\begin{tikzpicture}[->, >=stealth, thick]
...
```{tikz, fig3, echo=FALSE, fig.align = 'center', out.width = '90%'}
\begin{tikzpicture}[->, >=stealth, auto, semithick]

\node[draw, circle, thick, fill=red!40, minimum size=40pt] (A) at (0,0) {0};
\node[draw, circle, thick, fill=yellow!40, minimum size=40pt] (B) at (5,0) {1};
\node[draw, circle, thick, fill=yellow!40, minimum size=40pt] (C) at (8,0) {2};
\node[draw, circle, thick, fill=yellow!40, minimum size=40pt] (D) at (11,0) {3};
\node[draw, circle, thick, fill=yellow!40, minimum size=40pt] (E) at (14,0) {4};
\node[draw, circle, thick, fill=green!40, minimum size=40pt] (F) at (19,0) {5};

\path (A) edge[loop above] node {~} (A);
\path (F) edge[loop above] node {~} (F);
\path (B) edge[bend left] node[above] {$p$} (C);
\path (C) edge[bend left] node[below] {$q$} (B);
\path (C) edge[bend left] node[above] {$p$} (D);
\path (D) edge[bend left] node[below] {$q$} (C);
\path (D) edge[bend left] node[above] {$p$} (E);
\path (E) edge[bend left] node[below] {$q$} (D);
\path (E) edge[bend left] node[above] {$p$} (F);
\path (A) edge[loop above] node {1} (A);
\path (F) edge[loop above] node {1} (F);
\path (B) edge[bend left] node[below] {$q$} (A);
...
```{tikz, fig4, echo=FALSE, fig.align = 'center', out.width = '90%'}
\begin{tikzpicture}[->, >=stealth, auto, semithick, node distance=2.8cm, every node/.style={scale=0.9}]

% Nós na ordem desejada: 4, 2, 7, 1, 3, 5, 6
\node[draw, circle, thick, fill=yellow!40, minimum size=40pt] (4) {4};
\node[draw, circle, thick, fill=orange!40, minimum size=40pt, right of=4] (2) {2};
\node[draw, circle, thick, fill=orange!40, minimum size=40pt, right of=2] (7) {7};
\node[draw, circle, thick, fill=green!40, minimum size=40pt, right of=7] (1) {1};
\node[draw, circle, thick, fill=green!40, minimum size=40pt, right of=1] (3) {3};
\node[draw, circle, thick, fill=green!40, minimum size=40pt, right of=3] (5) {5};
\node[draw, circle, thick, fill=blue!30, minimum size=40pt, below of=3, yshift=-2cm] (6) {6};

% Transições:

% Estado 1 → 3
\path (1) edge[bend left=15] node[below] {1} (3);

% Estado 2 → 2 e 7
\path (2) edge[loop above] node {1/3} (2);
\path (2) edge[bend left=15] node[above] {2/3} (7);

% Estado 3 → 3 e 5
\path (3) edge[loop above] node {1/2} (3);
\path (3) edge[bend left=15] node[above] {1/2} (5);

% Estado 4 → 2 e 4
\path (4) edge[loop above] node {1/2} (4);
\path (4) edge[bend left=15] node[below] {1/2} (2);

% Estado 5 → 1, 3, 5
\path (5) edge[loop above] node {1/3} (5);
\path (5) edge[bend left=40] node[below] {1/3} (1);
\path (5) edge[bend left=15] node[below] {1/3} (3);

% Estado 6 → 6
\path (6) edge[loop above] node {1} (6);

% Estado 7 → 1, 2, 7
\path (7) edge[loop above] node {1/2} (7);
\path (7) edge[bend right=15] node[above] {1/4} (1);
\path (7) edge[bend left=10] node[below] {1/4} (2);
...
# Criar os dados como data frame {#criar-os-dados-como-data-frame}
tabela <- data.frame(
  Pai = c("Superior", "Médio", "Inferior"),
  `Superior` = c(0.45, 0.05, 0.01),
  `Médio` = c(0.48, 0.70, 0.50),
  `Inferior` = c(0.07, 0.25, 0.49)
)

# Detectar o formato de saída {#detectar-o-formato-de-saida}
output_format <- knitr::opts_knit$get("rmarkdown.pandoc.to")

# Gerar a tabela corretamente para HTML ou LaTeX {#gerar-a-tabela-corretamente-para-html-ou-latex}
if (output_format == "html") {
  kable(tabela, format = "html", align = "c") %>%
    add_header_above(c(" " = 1, "Filho" = 3)) %>%
    kable_styling(full_width = FALSE, position = "center")
} else {
  kable(tabela, format = "latex", booktabs = TRUE, align = "c") %>%
    add_header_above(c(" " = 1, "Filho" = 3))
}
...
\]
O grafo é
```{tikz, fig5, echo=FALSE, fig.align = 'center', out.width = '40%'}
\begin{tikzpicture}[->, >=stealth, auto, semithick]

\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (1) at (0,0) {1};
\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (2) at (-3,-5) {2};
\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (3) at (3,-5) {3};

\path[line width=1.5pt] (1) edge[loop above] node {0.45} (1);
\path[line width=1.5pt] (2) edge[loop left] node {0.70} (2);
\path[line width=1.5pt] (3) edge[loop right] node {0.49} (3);

% Draw arrow
%\draw[line width=1pt] (1) -- node[midway, above] {0.48} (2);
%\draw[line width=2pt] (1) -- node[midway, right] {0.07} (3);
%\draw[line width=2pt] (2) -- node[midway, above] {0.05} (1);
%\draw[line width=2pt] (2) -- node[midway, above] {0.25} (3);
%\draw[line width=2pt] (3) -- node[midway, above] {0.01} (1);
%\draw[line width=2pt] (3) -- node[midway, above] {0.50} (2);

\path[line width=1.5pt] (1) edge[bend left] node[left] {0.48} (2);
\path[line width=1.5pt] (1) edge[bend left] node[right] {0.07} (3);
\path[line width=1.5pt] (2) edge[bend left] node[left] {0.05} (1);
\path[line width=1.5pt] (2) edge[bend left] node[below] {0.25} (3);
\path[line width=1.5pt] (3) edge[bend left] node[right] {0.01} (1);
\path[line width=1.5pt] (3) edge[bend left] node[below] {0.50} (2);
...
```{tikz, Ex2.1ExerAlfredo, echo=FALSE, fig.align = 'center', out.width = '40%'}
\begin{tikzpicture}[->, >=stealth, auto, semithick]

\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (1) at (-3,0) {1};
\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (2) at (3,0) {2};
\node[draw, circle, thick, fill=blue!10, minimum size=40pt] (3) at (0,-3) {3};

\path (1) edge[bend left=15] node[above] {$1-p$} (2);
\path (2) edge[bend left=10] node[below] {1} (1);
\path (1) edge node[above] {$p$} (3);
\path (3) edge node[above] {$q$} (2);
\path (3) edge[loop below] node {$1-q$} (3);
...
```{tikz, fig6, echo=FALSE, fig.align = 'center', out.width = '50%'}
\begin{tikzpicture}[scale=1.2]
  % Eixos
  \draw[->] (0,0) -- (6,0) node[right] {$t$};
  \draw[->] (0,0) -- (0,3.2) node[above] {$X_t(\omega)$};

  % Linhas verticais (saltos)
  \draw[dashed] (1,0) -- (1,1);
  ...
  \draw[thick] (4,3) -- (5.5,3);

  % Pontos de salto (aberto à esquerda, fechado à direita)
  \fill (1,1) circle (1pt);
  \fill (2.5,2) circle (1pt);
  \fill (4,3) circle (1pt);
...
```{tikz, fig7, echo=FALSE, fig.align = 'center', out.width = '50%'}
\begin{tikzpicture}[->, >=stealth, auto, semithick]

\node[draw, circle, thick, fill=white, minimum size=40pt] (A) at (0,0) {$i$};
\node[draw, circle, thick, fill=white, minimum size=40pt] (B) at (5,0) {$i$};
\node[draw, circle, thick, fill=white, minimum size=40pt] (C) at (10,2) {$i+1$};
\node[draw, circle, thick, fill=white, minimum size=40pt] (D) at (10,-2) {$i-1$};

\draw (A) -- node[midway, above] {$T_i \sim Exp(\lambda_i+\mu_i)$} (B);
\path (B) edge[bend left] node[above] {$\lambda_i/(\lambda_i+\mu_i)$} (C);
\path (B) edge[bend right] node[below] {$\mu_i/(\lambda_i+\mu_i)$} (D);
...
# Gráfico {#grafico}
plot(t, W1, type = "l", col = "blue", lwd = 2,
     xlab = "Tempo", ylab = "W(t)", main = "Exemplo de duas trajectórias do processo de Wiener")
lines(t, W2, col = "orange", lwd = 2)
legend("topleft", legend = c("Trajectória 1", "Trajectória 2"),
...
knitr::include_graphics("figures/book1.png")
```
...
```{r, echo = FALSE, fig.align = 'center', out.width = '20%'}
knitr::include_graphics("figures/book3.png")
```
