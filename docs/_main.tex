...
Designa-se por \textbf{espaço amostral} o conjunto de todos os resultados possíveis de uma experiência aleatória, representado por \(\Omega\). No que se segue, consideramos que \(\Omega\) é um conjunto não vazio.
...
\item
  Se \({A_n}{n \in \mathbb{N}} \subseteq \mathcal{F}\), então \(\displaystyle \bigcup_{n \in \mathbb{N}} A_n \in \mathcal{F}\).
\end{enumerate}
...
\def\labelenumi{\roman{enumi})}
\item
  o valor esperado (ou valor médio) de \(X\) como
  \[
  E(X)=\int_{\Omega}{X}dP,
  \]
  se este integral existir e for finito;
\item
  a variância de \(X\) como
  \[
...
\begin{example}[Passeio aleatório]
...
\]
chama-se \textbf{passeio aleatório} (ou deslocação de um ponto no plano), tendo estado inicial \(X_0\) e distribuição \(p\).
\end{example}
...
Um estado \(i\) diz-se \textbf{recorrente} se, partindo de \(i\), a probabilidade de regressar a \(i\) em algum instante futuro é igual a \(1\).

Seja
\[
T_i = \inf \{ n \geq 1 : X_n = i \mid X_0 = i \},
\]
o \textbf{tempo de retorno ao estado \(i\)}, isto é, o primeiro instante (após a partida) em que a cadeia volta a atingir o estado \(i\).
...
Um estado \(i\) diz-se \textbf{transitório} se, partindo do estado \(i\), a probabilidade de retorno a \(i\) é menor que 1, isto é,
\[
P(T_i < +\infty) < 1,
\]
equivalentemente,
...
Considere-se a v.a. \(T_{ij}\), que representa o \textbf{tempo da primeira passagem} do estado \(i\) ao estado \(j\), definida por
\[
...
isto é, dizer que o estado \(i\) é \textbf{recorrente} significa que, partindo de \(i\), volta-se com probabilidade 1 a \(i\), num número finito de transições. Por outras palavras, há sempre a possibilidade de sair de \(i\) e chegar a \(i\) em \(n\) passos.
...
\begin{definition}[Estado transitório]
O estado \(i\) diz-se \textbf{transitório} se a probabilidade de voltar a \(i\), partindo de \(i\), for estritamente menor que 1, ou seja:
\[
\sum_{n=1}^{+\infty} f_{ii}^n < 1.
\]
Isto significa que, partindo de \(i\), existe uma probabilidade positiva de \textbf{não regressar} a \(i\). Em termos da variável aleatória \(T_{ii}\), isto é equivalente a:
\[
...
Uma cadeia de Markov com espaço de estados \(E\) diz-se \textbf{irredutível} se, para quaisquer estados \(i, j \in E\), existe um número natural \(n \geq 1\) tal que
\[
...
Em termos informais, isto significa que todos os estados da cadeia \textbf{comunicam entre si}, ou seja, é possível ir de qualquer estado para qualquer outro (talvez em vários passos), com probabilidade positiva.
\end{definition}
...
Uma \textbf{classe fechada} é uma classe de comunicação \(C\) tal que, se \(i \in C\) e \(P_{ij} > 0\), então \(j \in C\).
...
\item
  \textbf{Passo 2:} inclui-se em \(C\) todos os estados \(k\) para os quais \(P_{jk}>0\), \(\forall ~j \in C\), isto é, \(i \longrightarrow k\) (propriedade da transitividade);
\item
...
\section{Processo de Poisson homogéneo}\label{processo-de-poisson-homoguxe9neo}
...
\subsection{Definição e equações de Chapman-Kolmogorov}\label{definiuxe7uxe3o-e-equauxe7uxf5es-de-chapman-kolmogorov}
...
\subsection{Equações diferenciais de processos de nascimento e morte}\label{equauxe7uxf5es-diferenciais-de-processos-de-nascimento-e-morte}
...
\chapter{Complementos de processos estocásticos}\label{complementos-de-processos-estocuxe1sticos}
...
\section{O integral de Itô}\label{o-integral-de-ituxf4}
