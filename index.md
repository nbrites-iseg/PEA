...
- **Estado 1:** é **absorvente**, pois $P_{11} = 1$ e não há saídas para outros estados.  
...
\]
e o vetor \(\pi=(\pi_j)_{j\in E}\) for uma distribuição de probabilidade (ou seja, \(\pi_j\ge 0\) e \(\sum_{j\in E}\pi_j=1\)), então \(\pi\) designa-se por **distribuição limite** da cadeia.
:::

$\,$

::: {.definition name="Distribuição estacionária"}
Um vetor \(\pi=(\pi_j)_{j\in E}\) chama-se **distribuição estacionária** da cadeia se
\[
...
(ii) $\pi$ é estacionária: $\pi = \pi \mathbb{P}$.
:::
...
(i) sse a distribuição estacionária $\pi$ existe e é solução do sistema de equações lineares
$$
\begin{cases}
